...
public class MessageService {
	...
	public List<Message> getAllMessagesForYear(int year) {
		List<Message> mfYear = new ArrayList<Message>();
	...
	public List<Message> getAllMessagesPaginnated(int start, int size){
		ArrayList<Message> mp = new ArrayList<>(messages.values());
		if(start+size>mp.size()) return new ArrayList<>();
		return mp.subList(start, start+size);
	}
